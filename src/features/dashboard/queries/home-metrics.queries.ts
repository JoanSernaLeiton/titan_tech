...
export async function listDailyEnergyInRange(
  range: DateRange
  ...
  for (const row of snapshots) {
    ...
    if (energyValue > current) byDay.set(dayKey, energyValue);
    perDeviceDay.set(row.deviceId, byDay);
  }

  const totals = new Map<string, { energyKwh: number; devices: Set<string> }>();
  for (const [deviceId, byDay] of perDeviceDay) {
    for (const [dayKey, energy] of byDay) {
      const bucket = totals.get(dayKey) ?? { energyKwh: 0, devices: new Set<string>() };
      bucket.energyKwh += energy;
      bucket.devices.add(deviceId);
      totals.set(dayKey, bucket);
    }
  }

  return Array.from(totals.entries())
    .map(([date, { energyKwh, devices }]) => ({
      date,
      energyKwh,
